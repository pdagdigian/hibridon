...
*
      ...
      if(flaghf) then
         spin = 0.5d0
c deleted next line - all pairs of jtot and jtotp must be included
c in sums in sigk and sigkkp (pjd)
c         fast = .false.
      end if
*
...
c alternate parity for delta(jtotp-jtot) even/odd (pjd)
         if (((jtp-jtpmin)/2)*2 .ne. (jtp-jtpmin)) then
c delta=odd:  switch parity of jtotp
c first check whether jlpar=1 or -1
            if (jtp+1 .eq. jjp) then
c jlpar=1, skip forward to index for jtotp, jlpar=-1
               jjp = jjp + nwaves
            else
c jlpar=-1:  skip backward to index for jtotp, jlpar=1
               jjp = jjp - nwaves
            end if
         end if
c end of added/modified code (pjd)
         iaddrp = jttble(jjp)
...
c set parity to be considered for jtot' (pjd)
      jlparp = jlpar
*
* fill buffer with required s' matrices
c parity for each jtot' needs to be kept track of in addsp
...
60    continue
c set parity to be considered for jtotp:
c   jlparp = jlpar for delta(jtot - jtotp) = even
c   jlparp = -jlpar for delta () = odd
      jlparp = jlpar
...
* find address of jtot' in s-matrix buffer
      jj = jtotp + 1
c alternate parity for delta(jtotp-jtot) even/odd
...
*.....read next s-matrix
*
250   call sread (0, sreal, simag, jtot, jlpar, nu,
     :            jq, lq, inq, ipack, jpack, lpack,
     :            1, mmax, nopen, length, ierr)
      if(ierr .eq. -1) then
...
c     for elastic (jlevel,inlevel) -> (jlevel,inlevel) transition
c
...
c.....ca is wavevector for initial state, ecol is collision energy
      ...
      fak=ang2c/ca**2
c
c.....print m -> m' differential cross sections for this batch of angles
c
      numjm = jlevlp + jlevel + 1
c
c     SUPPRESS PRINTING OF M-RESOLVED CROSS SECTIONS IN PRODUCTION RUNS (immprt = 0)
      if (immprt .ne. 0) then
      write(2,302)
...
c
c calculates scattering amplitudes for given jtot and set
c of angles
c
c     author of original ampli program:  h.-j. werner
c     revised by p.j.dagdigian for helicity frame calculation
c     for the transition (j1,inlev1) -> (j2,inlev2)
c
c     revision date: 13-oct-2011
c
...
c
c     author of original ampli program:  h.-j. werner
c     revised by p.j.dagdigian for geomatric apse frame calculation
c     for the transition (j1,inlev1) -> (j2,inlev2)
c
c     revision date: 13-oct-2011
c
...
c
c     The angle beta is in radians
c     -----------------------------------------------------------------
c
