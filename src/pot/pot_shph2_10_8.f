c     ----------------------------------------------------------------- 
c     pot routine for the  SH+ - H2 PES computed by Dagdigian (2018)
c
c     points fit with LAmax = 10, LBmax = 8
c
c     to be used with the 3Sigma-1Sigma basis routine (in hiba3sg1sg.f)
c     data file used:  pot_shph2_10_8.dat
c
c     THIS VERSION OF THE POT ROUTINE HAS THE RIGHT R DEPENDENCE (R^-4) 
C     FOR THE ISOTROPIC (V000) TERM
c
c     pot matrix elements are computed assuming the angular dependence defined
c     in the appendix of green, jcp 62, 2271 (1975)
c
c     fixed problem of roundoff of angle values in transferring from molpro tables
c
c     revised from pot_c2hh2.f
c     current revision:  31-jan-2019 (p. dagdigian)
c     ------------------------------------------------------------------
...
c
     ...
     $  xmconv=0.0005485799094979479d0, sq4pi=3.544907701811032d0)
      end module mod_shph2
c     ----------------------------------------------------------------- 
c     loapot subroutine loads pot parameters for SH+ - H2 interaction
c     ----------------------------------------------------------------- 
...
C     the time.
      ...
      pot_name = 'Dagdigian SH+ - H2 RCCSD(T) AVTZ-F12 PES'
