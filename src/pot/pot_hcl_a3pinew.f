...
*  -------------------------------------------------------
      ...
      common /coered/ ered, rmu
      dimension rl(nl), dip1(nl), ddip1(nl), dip3(nl), ddip3(nl)
...
*  heere the morse parameters for hcl(C) from the av5z mrci+q calculations
      data de,r0,g0 /0.15471d0,2.5333d0,0.97659d0/
...
*  transition moments
     ...
     : 8.38e-6/
* NB this is NOT 3Sig-3Pi transition moment but rather avqz+diffuse
* CASCSF C-A diabatic mixing (in atomic units)
      data dip3 /
     : 1.6988851d-2, 1.8165661d-2, 2.0790740d-2, 2.2010943d-2,
     : 2.1297698d-2, 2.0694027d-2, 2.0390575d-2, 2.0194349d-2,
     : 1.9993037d-2, 1.9313262d-2, 1.8139744d-2, 1.6577128d-2,
     : 1.4786701d-2, 1.2974573d-2, 9.8950803d-3, 7.7195055d-3,
     : 5.9986791d-3, 4.5210334d-3, 3.3234125d-3, 2.3916469d-3,
     : 1.1790552d-3, 5.4278195d-4, 2.2298441d-4, 6.8964836d-5,
     : 2.5292745e-22/
...
*  interpolate the coefficients for spline expansion of 3Sig-3Pi
*  transition moment
*  (in fact C-A configurational coupling)

        nlm1 = nl -1
        ddip3f =( dip3(nl) - dip3(nlm1)) / (rl(nl) - rl(nlm1))
        ddip3i =( dip3(2) - dip3(1)) / (rl(2) - rl(1))
        call dspline( rl, dip3(1), nl, ddip3f, ddip3i, ddip3)
...
* calculate morse ground state wavefunction for C1Pi state
      rmlmda = two*rmu
      ...
      p = sqrt(g0*dexp(arg))
* here are the statements for harmonic oscillator functions
*      alph=21.8296d0
...
* this channel
      ppi=float(ibran)/20.d0
      wf(6)=2*rmlmda*p*dipfunc
* put some initial flux into 3rd channel (3Pi1)
      wf(3)=ppi*rmlmda*p*one*650/219474.6
* put some initial flux into 1st channel (3Sig1)
      wf(1)=-ppi*rmlmda*p*one*650/219474.6


      nch=6
c JK commented out   call mxma(t,1,nch,wf,1,nch,sc,1,nch,nch,nch,1)
c JK mxma call replaced with dgemm call
      call dgemm('n','n',nch,1,nch,1.d0,t,nch,wf,nch,
     : 0.d0,sc,nch)
