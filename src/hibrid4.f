...
c     ------------------------------------------------------------------
c     Function to return offset of wfu file for recrod #irec (stream IO)
c
c     Written by: Qianli Ma
c     Latest revision: 20-apr-2012
c
c     This function needs nchwfu, ipos2 and ipos3 from the cowave common
c     blocks.  These variables are set by waverd.
c
c     The stream IO counterpart for `dbrr(,,,irec)` is `read
c     (,pos=wavesk(irec))...
c     ------------------------------------------------------------------
      implicit double precision (a-h, o-z)
      integer iwavsk
...
c     Length for record 1 (at the beginning of the file)
      lr1 = 136 * sizeof(char_t) + (10 + 3 * nchwfu) * sizeof(int_t)
     $     + (4 + nchwfu) * sizeof(dble_t)
c     Length for each block written by the Airy and LOGDpropagator
      if (inflev .eq. 0) then
...
c     within the logd range of the file
         iwavsk = lr1 + (irecr - 4) * lrlogd + 1
         goto 100
      else
c     airy range of the file
         iwavsk = lr1 + nrlogd * lrlogd
     $        + (irecr - 4 - nrlogd) * lrairy + 1
         goto 100
      end if
c     should never reach here if function called properly
...
c
c     Major revision: 16-mar-2012 by Q. Ma
c     Use stream I/O for smaller file size and better compatibility
c
...
c     The three variables below are used to determine the (machine
c     dependent) size of the built-in types
      character char_t
      integer int_t
      double precision dble_t
c
      ...
      zero=0.d0
      izero=0
c     Size limit of wfu file is not necessary when stream I/O is used.
c     However, if 32-bit integer is used, files exceeding 2GB cannot be
c     seeked properly.  With ifort, '-i8' option will force using 64-bit
c     integers.
c
c$$$      if (nchtop .gt. 1000) then
c$$$         write (6, 60) nchtop
c$$$ 60      format(/' *** NCHTOP=',i3,
c$$$     :        ' EXCEEDS MAX FOR STORAGE OF WAVEFUNCTION; ABORT ***')
c$$$         call exit
c$$$      endif
c
      nchwfu = nch
      ...
      ipos3 = -1
      nrlogd = 0
c     Mark the position of the EOF of the WFU file in order to by pass
c     (likely) a bug in the intel compiler that INQUIRE does not return
c     the proper offset
      iendwv = 1
c     Write magic number
...
* transform wave function into local basis
          if (propf) then
c JK commented out            call mxma(scmat,1,nch,psir,1,nch,scmat2,1,nch,nch,nch,2)
            call dgemm('n','n',nch,2,nch,1.d0,scmat,nch,psir,nch,0.d0,
     :                 scmat2,nch)
...
c JK commented out             call mxma(scmat3,1,nch,psii(ipoint),1,nch,psii,1,
c JK commented out    :               nch,nch,nch,2)
             call dgemm('n','n',nch,2,nch,1.d0,scmat3,nch,psii(ipoint),
     :                  nch,0.d0,psii,nch)
* 1st two column of psii now contain G-tilde(A-B)*psi-tilde(b)
* if photodissociation, subtract off mu-tilde(a,b) from real part
c JK commented out            if (photof) call vadd(mone,psii,1
c JK commented out     :                          ,psir(noffset),1,nch)
c JK replaced call to vadd with daxpy
            if (photof) call daxpy(nch,dfloat(mone),
     :           psir(noffset),1,psii,1)
...
c JK commented out            if (photof) call vadd(mone,psir(ipoint),1,sc9,1,nch)
c JK replaced call to vadd with call to daxpy
            if (photof) call daxpy(nch,dfloat(mone),sc9,1,
     :                            psir(ipoint),1)
...
300         continue
c JK commented out   call mxma(scmat,1,nch,psir,1,nch,scmat2,1,nch,nch,nch,4)
            call dgemm('n','n',nch,4,nch,1.d0,scmat,nch,psir,nch,0.d0,
     :                  scmat2,nch)
            call dcopy(2*nch,scmat2,1,psii(ipoint),1)
...
c JK commented out            if (photof) call vadd(mone,scmat2(2*nch+1),1
c JK commented out     :                          ,psir(noffset),1,nch)
c JK replaced call to vadd with call to daxpy
            if (photof) call daxpy(nch,dfloat(mone),
     :                       psir(noffset),1,scmat2(2*nch+1),1)
...
c JK commented out            if (photof) call vadd(mone,scmat2,1,sc9,1,nch)
c JK replaced call to vadd with call to daxpy
            if (photof) call daxpy(nch,dfloat(mone),sc9,1,scmat2,1)
...
c JK commented out          call mxma(scmat,nch,1,psii,1,nch,psir,1,nch,nch,nch,4)
          call dgemm('t','n',nch,4,nch,1.d0,scmat,nch,psii,nch,
     :               0.d0,psir,nch)
...
* 13p or 2s-2p
              ...
              if (ibasty .eq. 7) then
c JK commented out                call mxma(ttrans,nch,1,psir,1,nch,
c JK commented out     :                    psii,1,nch,nch,nch,4)
                call dgemm('t','n',nch,4,nch,1.d0,ttrans,nch,
     :                     psir,nch,0.d0,psii,nch)
...
* 13p or 2s-2p
              if (ibasty .eq. 7) then
c JK commented out                call mxma(ttrans,1,nch,psir,1,nch,
c JK commented out     :                    psii,1,nch,nch,nch,4)
                call dgemm('n','n',nch,4,nch,1.d0,ttrans,
     :                      nch,psir,nch,0.d0,psii,nch)
* psii now contains this transformed wavefunction and derivative
* copy these back to psir
                call dcopy(4*nch,psii,1,psir,1)
              endif
...
c JK commented out          call mxma(sr,1,nch,psir,1,nch,si,1,nch,nch,nch,1)
          call dgemm('n','n',nch,1,nch,1.d0,sr,nch,
     :              psir,nch,0.d0,si,nch)
          do 160 i=1, nj
            sc(i)=si(nlist(i))
160       continue
...
c
c     Subroutine to readin and print out adiabatic energies from a wfu
c     file.  wfu files with only adiabatic energy information
c     (wrsmat=.f. when performaing wavefunction calculations) can be
c     supported.
c
c     Adiabatic energies as a function of r will be saved to
c     filnam.eadiab, with columns the adiabatic bender curves.  r is
c     stored in the first column.
c
c     author: qianli ma
c     current revision: 20-apr-2012
c
c     ------------------------------------------------------------------
      use constants
      implicit double precision (a-h, o-z)
