...
*  interval
        if (photof) then
c JK commented out   call mxma(tmat,1,nmax,q,1,nphoto,w,1,nmax,nch,nch,nphoto)
c JK replaced above mxma call  with BLAS call
         call dgemm('n','n',nch,nphoto,nch,1.d0,tmat,nmax,q,
     .              max(nch,nphoto),0.d0,w,nmax)
...
c JK commented out          call vadd(1,q,1,gam1,1,nqmat)
c JK replaced call to vadd with call to daxpy
          call daxpy(nqmat,1.d0,gam1,1,q,1)
*  then premultiply by Z(0,rlast,rnext)
c JK commented  out  call mxma(z,1,nmax,q,1,nphoto,tmat,1,nmax,nch,nch,nphoto)
          call dgemm('n','n',nch,nphoto,nch,1.d0,z,
     .         nmax,q,max(nphoto,nch),0.d0,tmat,nmax)
...
c JK commented out            call vadd(1,gam2(ind),1,cc,1,nch)
c JK replaced call to vadd with call to daxpy
            call daxpy(nch,1.d0,cc,1,gam2(ind),1)
...
* if photodissociation calculation, also transform gamma2 to free basis
      if (photof) then
c JK comment     call mxma(vecnow,1,nmax,q,1,nphoto,w,1,nmax,nch,nch,nphoto)
       call dgemm('n','n',nch,nphoto,nch,1.d0,vecnow,
     .            nmax,q,max(nch,nphoto),0.d0,w,nmax)           
        ind=1
        jnd=1
        do 265 i=1,nphoto
...
c JK commented out   call mxma(vecnow,1,nmax,q,1,nch,scr,1,nch,nch,nch,2)
      call dgemm('n','n',nch,2,nch,1.d0,vecnow,
     .           nmax,q,max(nch,2),0.d0,scr,nch)
